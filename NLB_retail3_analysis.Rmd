...
# TODO also figure out what we must do or how we need to clean out our data with "I do not know" answers like Nevem which is equal to -99
```
...
### H: Up to 55 % of young people would be prepared to pay more for sustainable products.
...
# 2. Data Cleanup
nrow(mydata) # initial max. of people

myDataPriceNormal <- mydata %>% replace_with_na(replace = list(Q18 = c(-1)))
myDataPriceNormal <- drop_na(myDataPriceNormal)
myDataPriceNormal <- as.numeric(as.character(myDataPriceNormal[ ,"Q18"]))

length(myDataPriceNormal)

myDataPriceSustainable <- mydata %>% replace_with_na(replace = list(Q24 = c(-1)))
myDataPriceSustainable <- drop_na(myDataPriceSustainable)
myDataPriceSustainable <- as.numeric(as.character(myDataPriceSustainable[ ,"Q24"]))
...
# VERDICT: We can overthrow null hypothesis and accept the H1 hypothesis, p-value = 0.003506.
...
# Cleaning data for sustainability views

myDataCorr1 <- mydata %>% replace_with_na(replace = list(Q19a = c(-1, -5), Q19b = c(-1, -5), Q19c = c(-1, -5), Q19d = c(-1, -5), Q19e = c(-1, -5)))
myDataCorr1 <- drop_na(myDataCorr1)
...
employmentData <- myDataCorr1 %>% replace_with_na(replace = list(XDS2a4 = c(-1, -5)))
employmentData <- drop_na(employmentData)
employmentData <- as.numeric(as.character(employmentData[ ,"XDS2a4"]))

length(employmentData)

is.numeric(employmentData)
...
viewsOnSustainabilityQ19a <- as.numeric(as.character(myDataCorr1[, "Q19a"])) 
viewsOnSustainabilityQ19b <- as.numeric(as.character(myDataCorr1[, "Q19b"]))
viewsOnSustainabilityQ19c <- as.numeric(as.character(myDataCorr1[, "Q19c"]))
viewsOnSustainabilityQ19d <- as.numeric(as.character(myDataCorr1[, "Q19d"]))
viewsOnSustainabilityQ19e <- as.numeric(as.character(myDataCorr1[, "Q19e"]))
length(viewsOnSustainabilityQ19e)

# Here i just calculated the average of all 5 questions when we measured sustainability 
viewsOnSustainability <- as.numeric(as.character(myDataCorr1[, "Q19e"]))
...
viewsOnSustainabilityNew$EmployedF
...
myDataH7 <- mydata %>% replace_with_na(replace = list(Q18 = c(-1)))
myDataH7 <- drop_na(myDataH7)
myDataH7 <- as.numeric(as.character(myDataH7[ ,"Q22"]))

myDataH7

myDataH7SampleSize <- length(myDataH7) # max. of people on clean data
myDataH7SampleSize

# 3. Getting the relevant data
counterH7 <- 0

for(i in 1:myDataH7SampleSize) {
  if(myDataH7[i] == 1) {
...
# VERDICT: We can overthrow null hypothesis and accept the H1 hypothesis, p-value = 0.002292.
...
myDataPriceFourQ <- data.frame(myDataPrice[, 111], myDataPrice[, 112], myDataPrice[, 113], myDataPrice[, 114])
# delete these 3 of them
myDataPriceFourQ[8, ]
myDataPriceFourQ[81, ]
...
## 10. Market potential - WE DO NOT NEED
...
## 11. Buyer behaviour - WE DO NOT NEED
