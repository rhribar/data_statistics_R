...
finalNA <- data.frame(dostopnostNA, cenaNA, ugledNA, odgovornostNA)
...
#### This is acceptable because all of these values are above 5.
...
```
#### There is less than expected people in group one that answered NO. Second group same. 3rd no difference.
...
```{r}
chisq_resultsEdu <- chisq.test(segmentationData$EducationF, as.factor(segmentationData$Clustering_kmeans))
chisq_resultsEdu

addmargins(chisq_resultsEdu$observed)

round(chisq_resultsEdu$res, 2) ### do fisher test

fisher.test(segmentationData$EducationF, as.factor(segmentationData$Clustering_kmeans),simulate.p.value=TRUE,B=1e7) # very intensive

# use this if error : Error in fisher.test(segmentationData$EducationF, as.factor(segmentationData$Clustering_kmeans)) : FEXACT error 7(location). LDSTP=18630 is too small for this problem,

```

#### Not statistically significant because values are too low.
```{r}
chisq_resultsNeo<- chisq.test(segmentationData$NeobankUseF, as.factor(segmentationData$Clustering_kmeans))
chisq_resultsNeo

addmargins(chisq_resultsNeo$observed)

round(chisq_resultsNeo$expected, 2)

round(chisq_resultsNeo$res, 2)
```
...
```{r}
chisq_resultsType <- chisq.test(segmentationData$BankTypePreferenceF, as.factor(segmentationData$Clustering_kmeans))
chisq_resultsType

addmargins(chisq_resultsType$observed)

round(chisq_resultsType$expected, 2)

round(chisq_resultsType$res, 2)
```
#### None of the groups are statistically significant - values are too low.
#### I do not think this variable can be used for segmentation because not all respondents answered this question.
...
```{r}
chisq_resultsKids <- chisq.test(segmentationData$KidsF, as.factor(segmentationData$Clustering_kmeans))
chisq_resultsKids

addmargins(chisq_resultsKids$observed)

round(chisq_resultsKids$res, 2)

```
#### Values are too low to be statistically significant.

```{r}
chisq_resultsSal <- chisq.test(segmentationData$SalaryF, as.factor(segmentationData$Clustering_kmeans))
chisq_resultsSal

addmargins(chisq_resultsSal$observed)

round(chisq_resultsSal$res, 2)

# fix the test here, fischer test should be used
# recode, then instead of chi use OWA ...

# SalaryF
segmentationData$SalaryF_aov <- factor(segmentationData$Q30, 
                            levels = c(1, 2, 3, 4, 5, 6, 7), 
                            labels = c("0", "499", "999", "1499", "1999", "2499", "2500"))

segmentationData$SalaryF_aov <- as.numeric(as.character(segmentationData$SalaryF_aov))


segmentationData$SalaryF_aov

aov(segmentationData$SalaryF_aov ~ as.factor(segmentationData$Clustering_kmeans))
```
#### Values are too low to be statistically significant.
